...
var Enemy = function (name, hp) {
    this.name = name;
    this.hpMax = Math.floor(hp * hpBonus);
    this.hp = Math.floor(hp * hpBonus);
    this.resetHp = function() {
        this.hp = Math.floor(this.hpMax * hpBonus);
    } 
};
...
var charArray = [froggyg, lonestar, leon, scoop, gnaw, raelynn, ayla, clunk, voltar, coco, skolldir, yuri, derpl, vinnie, genji, swiggins, rocco];
...
function calcSolar(amount) {
    solarTot += Math.floor(amount * solarBonus);
    bSolar = solarTot % 100;
    $("#bSolar").text(bSolar);
...
function calcPresBonus() {
    presBonus = tokenValue * presToken;
}
    ...
    if (char.purchased == false) {
        ...
        if (char.type == "normal") {
            if (solarTot >= char.cost) {
                solarTot -= char.cost;
                dps += Math.floor(char.dps * presBonus);
                $("#dps").text(dps);
                calcSolar(0);
                char.purchased = true;
                charBought += 1;
...
function skillBuy(skill) {
    if (skill.purchased == false) {
        if (solarTot >= skill.cost) {
            ...
            if (skill.char != "user") {
                dps += Math.floor(((skill.char.dps * skill.dpsPerc) - skill.char.dps) * presBonus);
                
                $("#dps").text(dps);
            }
            else {
                dpc = Math.floor(skill.dpcPerc*presBonus);
                $("#dpc").text(dpc);
            }
            calcSolar(0);
            return true;
...
function nextEnemy() {
    ...
    curEnemy.hp = Math.floor(curEnemy.hpMax * hpBonus);
...
$(document).ready(function() {
        ...
        $('body').on('click', ".popover-content", function(e){
            switch (curPage) {
                case "page1":
                    ...
                    $(this).html("<div id='page2' class='tutdiv'>Your goal is to defeat all of the enemies. Characters help you do this by doing damage for you over time, and upgrades help you do more damage at once. After defeating all of the enemies, you can choose to prestige for prestige tokens or another bonus, but your progress is reset. This game is based on <a href='http://awesomenauts.com/'>Awesomenauts</a>, by Ronimo Games.</div><span class='continue'><em>Click to close...</em></span>");
                    curPage = "page2"
                    break;
                case "page2":
                    $tutBtn.popover('hide');
    ...
    $(".charPicture").tooltip({title: "Click to purchase", placement: "right", delay: {"show": 500, "hide": 100}});
    $(".upgradeRight").tooltip({html: "true", placement: "right", delay: {"show": 300, "hide": 100}});
        $(".upgradeLeft").tooltip({html: "true", placement: "left", delay: {"show": 300, "hide": 100}});
    ...
    var $presOverlay = $("#prestige-overlay-outer");
    var $content = $("#content");

    $(".prestige-button").click(function() {
        presType = this.id;
        $presOverlay.fadeIn(500);
        $presOverlay.removeClass("hidden");
        $content.addClass("blur");
    });
    $(".prestige-yes").click(function() {
        if (presType == "standard" && presCount < 10) {
            presCount += 1;
            hpBonus *= 1.1;
            solarBonus *= 1.05;
            }
            ...
            curEnemy = worm;
            curEnemy.resetHp();
            solarTot = dps = charBought = 0;
            dpc = Math.floor(presBonus);
            calcEnemy();
            calcSolar(0);
            for (var i=0; i<charArray.length; i++) {
                charArray[i].purchased = false;
                }
           $(".charPicture").addClass("locked");
           $("#presCount").text(presCount);
           $("#presTokens").text(presToken);

           $("#presBonus").text((Math.round((presBonus*100%100)*10))/10);
           $content.removeClass("blur");
           $presOverlay.fadeOut(500);
    });
    $(".prestige-no").click(function() {
        $content.removeClass("blur");
        $presOverlay.fadeOut(500);
    });
});
